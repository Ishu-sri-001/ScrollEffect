  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      planesRef.current.forEach((plane, index) => {
        ...
        plane.position.y = baseY + scrollOffset * 2;
        ...
        const t = 1 - Math.min(distanceFromTarget / activationZone, 1);
        ...
        if (t > 0) {
          ...
          for (let i = 0; i < posAttr.count; i++) {
            ...
            const screenZCurve = -Math.sin(screenNormalizedY * Math.PI) * curveIntensity * t;
            ...
            if (colIndex === 0) {
              ...
              deformY = screenYWave * leftEdgeFactor * yWaveIntensity * t;
            } else {
              // Right plane - wave only on right outer edge  
              const rightEdgeFactor = Math.pow(normalizedX, 2);
              // Y wave follows the same screen curve pattern
              const screenYWave = Math.sin(screenNormalizedY * Math.PI * 2) * 0.5; // More frequent wave
              deformY = screenYWave * rightEdgeFactor * yWaveIntensity * t;
            }
