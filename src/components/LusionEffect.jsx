...
const ImageGrid = () => {
  ...
  useEffect(() => {
    ...
    const imageData = [
      { path: '/zeni.webp', isVertical: true },   
      { path: '/aot.jpg', isVertical: false },  
      { path: '/pic3.jpg', isVertical: false },  
      { path: '/levi.jpg', isVertical: true },   
      { path: '/pic1.webp', isVertical: true },   
      { path: '/pic6.jpg', isVertical: false }  ,
      { path: '/pic1.jpg', isVertical: false },   
      { path: '/naruto.jpg', isVertical: true }   ,
      { path: '/levi.jpg', isVertical: true },   
      { path: '/zeni.webp', isVertical: true }  ,
      { path: '/pic5.jpg', isVertical: false },   
      { path: '/pic6.jpg', isVertical: false }  ,
      { path: '/pic1.webp', isVertical: true },   
      { path: '/levi.jpg', isVertical: true }  ,
       { path: '/pic3.jpg', isVertical: false }  ,
  ...
  uniforms: {
    ...
    uRadius: { value: 0.1 }, // adjust for corner roundness
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      float alpha = 1.0;
      vec2 bl = vec2(r, r); // bottom left
      vec2 br = vec2(uResolution.x - r, r); // bottom right
      vec2 tl = vec2(r, uResolution.y - r); // top left
      vec2 tr = vec2(uResolution.x - r, uResolution.y - r); // top right

      if (pos.x < r && pos.y < r) {
        // bottom-left corner
        if (distance(pos, bl) > r) discard;
      } else if (pos.x > uResolution.x - r && pos.y < r) {
        // bottom-right
        if (distance(pos, br) > r) discard;
      } else if (pos.x < r && pos.y > uResolution.y - r) {
        // top-left
        if (distance(pos, tl) > r) discard;
      } else if (pos.x > uResolution.x - r && pos.y > uResolution.y - r) {
        // top-right
        if (distance(pos, tr) > r) discard;
      }
  ...
  transparent: true,
    ...
    const animate = () => {
      ...
      const isScrollingStopped = timeSinceLastScroll > 1;
...
if (isScrollingStopped) {
  deformationFactorRef.current = lerp(deformationFactorRef.current, 0, 0.08); // slower decay
  yWaveFactorRef.current = lerp(yWaveFactorRef.current, 0, 0.08);
} else {
  deformationFactorRef.current = lerp(deformationFactorRef.current, 1, 0.12); // fast ramp up
  yWaveFactorRef.current = lerp(yWaveFactorRef.current, 1, 0.12);
}
